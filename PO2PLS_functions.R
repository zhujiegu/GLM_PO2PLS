...
variances_inner.po2m_test <- function(fit, X, Y){
  tmp.Estep <- E_step(X, Y, fit$par)
  with(tmp.Estep,
       Stt%*%solve(fit$par$SigH) - solve(fit$par$SigH)%*%
         (Sut%*%t(Sut) - Sut%*%Stt%*%fit$par$B - t(Sut%*%Stt%*%fit$par$B) + crossprod(Stt)%*%fit$par$B^2)%*%
         solve(fit$par$SigH)) %>%
    multiply_by(nrow(X)) %>% solve %>% diag %>% abs %>% raise_to_power(0.5)
}
