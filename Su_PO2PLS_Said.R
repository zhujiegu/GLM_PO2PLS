...
E_step <- function(X, Y, Z, params){
  ...
  GammaEFG <- Gamma
  GammaEFG[1:p,c(1:r,2*r+1:rx)] <- 1/sig2E* GammaEFG[1:p,c(1:r,2*r+1:rx)]
  GammaEFG[p+1:q,c(r+1:r,2*r+rx+1:ry)] <- 1/sig2F* GammaEFG[p+1:q,c(r+1:r,2*r+rx+1:ry)]
  GammaEFG[p+q+1,1:(2*r)] <- 1/sig2G* GammaEFG[p+q+1,1:(2*r)]
  ...
  logdet <- log(det(diag(2*r+rx+ry) + GGef%*%SigmaM))+p*log(sig2E)+q*log(sig2F)+log(sig2G)
  ## representation of SigmaXY %*% invS
  XYZinvS <- ssq(cbind(X/sqrt(sig2E), Y/sqrt(sig2F), Z/sqrt(sig2G)))
