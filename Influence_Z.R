...
params_true <- generate_params(x, y, z, r, rx, ry, alpha = 0.1, type='random')
...
for(i in 1:50){
  ...
  fit <- Su_PO2PLS(X, Y, Z, 2, rx, ry, steps = 3000, init_param = "o2m")
  pre_z <- with(fit$params, X_t%*%W%*%t(a) + Y_t%*%C%*%t(b))
  r2_su[i] <- with(glm(Z_t~pre_z) %>% summary, 1 - deviance/null.deviance)
  # 
  # 
  # fit_o2 <- PO2PLS::PO2PLS(X, Y, 2, rx, ry, steps = 3000, init_param = "o2m")
